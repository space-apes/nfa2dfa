/*
   ...
   queue and a stack classes with standard functionality (pop,push,peek, enqueue, dequeue, isEmpty,getSize etc)

    convert2Set(inputArray) 
        removes repeat elements OR EMPTY STRINGS, of an array
    setAdd(item, inArray)
        only adds element if it is not already in array
    splitOrEmpty(inputString, limiterString)
        splits input string into array of strings at every point limiter string is encountered
        OR 
        if inputstring is empty string, return an empty array

        made this because empty strings were outputting arrays of 1 element 

    addUniqueState(stateObject, limitString, finiteAcceptorObject)
        this one is based off of the states .name member value and only adds 
        the state to the acceptor if some permutation of its name components isn't in it

        example: adding state1 with state1.name="q1-q2"
                attempting to add state2 with state2.name="q2-q1" FAIL    
